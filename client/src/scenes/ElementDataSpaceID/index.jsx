...
const ElementDataSpaceID = () => {
  ...
  const { elementDataSpaceID } = useParams();
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        ...
        const filteredFiles = response.data.filter(
          (file) => file.elementDataSpaceID === elementDataSpaceID && file.status === 'public'
        );
    ...
    fetchFiles();
  }, [elementDataSpaceID, token]);

  const handleFileClick = (fileID) => {
    navigate(`/elementdataspace/${elementDataSpaceID}/${fileID}`);
  };
